...
contract TournamentManagerInvariantTest is Test {
    ...
    function invariant_moneyConservation() public {
        ...
        if (handler.totalETHDeposited() > handler.totalETHWithdrawn()) {
            assertLe(totalETHInSystem, handler.totalETHDeposited() - handler.totalETHWithdrawn(), 
    ...
    function invariant_tokenConservation() public {
        ...
        if (handler.totalTokensDeposited() > handler.totalTokensWithdrawn()) {
            assertLe(totalTokensInSystem, handler.totalTokensDeposited() - handler.totalTokensWithdrawn(),
    ...
    function invariant_tournamentConsistency() public {
        ...
        for (uint256 i = 0; i < tournamentIds.length; i++) {
            ...
            if (status == TournamentManager.TournamentStatus.Completed && 
                handler.hasTournamentEnded(tid) && 
                ...
                if (prizesDistributed) {
                    ...
                    if (winners.length == 1) {
                        ...
                        assertEq(handler.getWinnerPrize(tid, 0), amountInTournament, 
                                "Single winner didn't get full prize");
                    } else if (winners.length == 2) {
                        // With 2 winners, they should get 50% and 30%
                        assertEq(handler.getWinnerPrize(tid, 0), amountInTournament * 50 / 100, 
                                "First winner didn't get 50%");
                        assertEq(handler.getWinnerPrize(tid, 1), amountInTournament * 30 / 100, 
                        ...
                        assertEq(handler.getWinnerPrize(tid, 1), amountInTournament * 30 / 100, 
                                "Second winner didn't get 30%");
                        assertEq(handler.getWinnerPrize(tid, 2), amountInTournament * 20 / 100, 
                                "Third winner didn't get 20%");
                    }
                }
            }
            ...
            if (tokenAddress != address(0) && status != TournamentManager.TournamentStatus.Cancelled) {
                ...
                if (!prizesDistributed) {
                    assertGe(IERC20(tokenAddress).balanceOf(address(tournamentManager)), 
                            amountInTournament, 
...
contract TournamentHandler is Test {
    ...
    function joinTournament(uint256 userIndex, uint256 tournamentIndex) public {
        ...
        try tournamentManager.getTournamentDetails(tournamentId) returns (
            ...
            if (status != TournamentManager.TournamentStatus.Open ||
    ...
    function submitScores(uint256 tournamentIndex) public {
        ...
        try tournamentManager.getTournamentDetails(tournamentId) returns (
            ...
            if (status != TournamentManager.TournamentStatus.InProgress ||
                block.timestamp > scoreSubmissionDeadline ||
                activePlayers == 0) {
                return;
            }
